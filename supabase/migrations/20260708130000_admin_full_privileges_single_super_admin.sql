-- Admin shares Super Admin CRM privileges; Super Admin remains a singleton role.
...
-- Org admins can update projects, wings, and unit types (not Super Admin only).
drop policy if exists "projects_update_super_admin" on public.projects;
...
drop policy if exists "wings_mutate_super_admin" on public.project_wings;
...
drop policy if exists "unit_types_mutate_super_admin" on public.project_unit_types;
...
drop policy if exists "negotiation_approvals_update_super_admin" on public.negotiation_approvals;
...
drop policy if exists "profiles_update_super_admin_any" on public.profiles;
create policy "profiles_update_org_admin_any"
on public.profiles
for update
using (public.is_org_admin())
with check (public.is_org_admin());
