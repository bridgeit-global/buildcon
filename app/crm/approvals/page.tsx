...
type ApprovalRecordFromDb = {
  ...
  profiles?: EmbedProfile | EmbedProfile[] | null;
};
  ...
  const loadRows = useCallback(async () => {
    ...
    const { data, error: readErr } = await supabase
      ...
      .select(
        ...
        units ( unit_code ),
        projects ( name ),
        profiles!negotiation_approvals_requested_by_fkey ( name )
      `
      )
    ...
    const mapped: ApprovalRow[] = (
      (data ?? []) as unknown as ApprovalRecordFromDb[]
    ).map((r) => {
      ...
      const profile = embedOne<EmbedProfile>(r.profiles);
      return {
        id: r.id,
